...
function App() {
	...
	const handleClickInputFile = (num) => {
		...
		if(num == 1)
			...
			inputFile = document.getElementById("fileInputTwo");
		else if(num == 3)
			inputFile = document.getElementById("fileInputThree");
		inputFile.click();
	}
	...
	const applyStyle = (styleNumber,classToAdd,styleNo) => {
		...
		for(let attri of dropboxParent.classList){
			if(attri.includes('style-') || attri.includes('styletwo-') || attri.includes("stylethree-")){
	...
	const handleLayoutChange = (e,index) => {
		...
		const dropboxParent = document.querySelector('.canvas .centeredDropbox');
		for(let attri of dropboxParent.classList){
			if(attri.includes('style-') || attri.includes('styletwo-') || attri.includes("stylethree-") || attri.includes("layout-")){
				dropboxParent.classList.remove(attri);
			}
		}
	}
	...
	return (
		<div className="container">
							...
							<div className="export">
								<div className="exportBtn">
									<button className='download'> 
										<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12.404 16.49V8.317m3.752 4.408s-2.528 3.764-3.752 3.764-3.748-3.764-3.748-3.764"></path><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3.154 12.404c0 6.937 2.313 9.25 9.25 9.25s9.25-2.313 9.25-9.25-2.313-9.25-9.25-9.25-9.25 2.313-9.25 9.25" clip-rule="evenodd"></path></svg>
										<div>
											<span>Download</span>
											<p>1x as PNG</p>
										</div>
									...
									<button className='copy'>
										<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="currentColor" fill-rule="evenodd"><path d="M11.82 6.02h.2c.16 0 .32.06.43.18l4.07 4.24c.1.11.16.26.16.41v7.48c0 1.98-1.62 3.61-3.6 3.64H6.49c-1.99-.04-3.58-1.68-3.54-3.66V9.59c.04-2 1.69-3.61 3.67-3.61l5.04-.01c.03-.01.06-.01.1-.01Zm-.61 1.21H6.66c-1.34 0-2.44 1.07-2.48 2.41v8.71a2.376 2.376 0 0 0 2.34 2.43l.11-.01h6.44a2.44 2.44 0 0 0 2.39-2.44l-.01-6.73h-1.62a2.684 2.684 0 0 1-2.67-2.68l-.01-1.73Zm1.2.67v1.05c0 .8.65 1.46 1.46 1.46l.95-.01-2.42-2.52Z"></path><path d="M15.949 2c-.04-.01-.07-.01-.11-.01h-5.16c-1.99 0-3.63 1.6-3.68 3.6V6.8h1.2V5.6a2.48 2.48 0 0 1 2.47-2.42l4.54-.01v1.72c0 1.47 1.19 2.67 2.66 2.67l1.61-.01v6.72c0 1.32-1.08 2.41-2.4 2.43H15.8v1.2l1.27-.01a3.67 3.67 0 0 0 3.596-3.65V6.76c0-.16-.07-.31-.17-.42l-4.08-4.25a.63.63 0 0 0-.44-.19l-.11-.01Zm.5 2.93-.01-1.06 2.41 2.51h-.96c-.81-.01-1.47-.67-1.47-1.47Z"></path></g></svg>

									</button>
								</div>
								<div className="settings">
									<button>
										<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd" d="M17.38 13.7c1.71 0 3.11 1.38 3.11 3.09 0 1.7-1.4 3.09-3.12 3.09s-3.12-1.39-3.12-3.1 1.39-3.1 3.11-3.1Zm0 1.5c-.89 0-1.62.71-1.62 1.59s.72 1.59 1.61 1.59c.88 0 1.61-.72 1.61-1.6s-.73-1.6-1.62-1.6Zm-7.31.88a.749.749 0 1 1 0 1.5H3.76c-.42 0-.75-.34-.75-.75 0-.42.33-.75.75-.75h6.3ZM6.1 3.98c1.71 0 3.11 1.39 3.11 3.09s-1.4 3.09-3.12 3.09-3.12-1.388-3.12-3.1c0-1.71 1.39-3.1 3.11-3.1Zm0 1.5c-.89 0-1.62.71-1.62 1.59s.72 1.59 1.61 1.59 1.61-.72 1.61-1.6c0-.89-.73-1.6-1.62-1.6Zm13.07.9a.749.749 0 1 1 0 1.5h-6.3c-.42 0-.75-.34-.75-.75 0-.42.33-.75.75-.75z"></path></svg>
									</button>
								</div>
							</div>
						</div>
			...
			<input type="file" onChange = {(e)=>handleFileChange(e,1)} id="fileInput" style={{display:"none"}}/>
			<input type="file" onChange = {(e)=>handleFileChange(e,2)} id="fileInputTwo" style={{display:"none"}}/>
			<input type="file" onChange = {(e)=>handleFileChange(e,3)} id="fileInputThree" style={{display:"none"}}/>
							...
							<div className="dropboxParent">
								<div className="dropbox">
									<div className={`centeredDropbox style-0 ${layoutsName}`} >
										...
										<div className="uploadBox" onClick={()=>handleClickInputFile(1)} >
										...
										<div className="uploadBox none" onClick={()=>handleClickInputFile(2)}>
											<div className='stack2 none'>
													{
														previewTwo &&
														<div className="imgContainer">
										...
										<div className="uploadBox none" onClick={()=>handleClickInputFile(3)}>
											<div className='stack2 none'>
													{
														previewThree &&
														<div className="imgContainer">
															<img src={previewThree} alt="preview" style={{filter:"blur(2em)"}}/>
														</div>
													}
											</div>
											<div className='stack1 none'>
				...
				<div className="scroller">
					{
						...
						<ScreenshotPosition preview = {preview} preview2 = {previewTwo} applyStyle={applyStyle}/>
					}
					{
						layoutsName === "layout-2" &&
						<MinimalPhonePosition preview = {preview} preview2 = {previewTwo} preview3 = {previewThree} applyStyle = {applyStyle}/>
					}
